<template>
    <v-footer fixed class="elevation-3" style="width: 100%">
        <v-col class="pa-0" color="grey lighten-5">
            ...
            <v-btn outlined class="footer-back btn-xs-only" style="text-transform: none" v-on:click="back(9)" v-show="icons > 0">
                <span class="footer-font-size">一覧</span>
            </v-btn>
            <v-btn outlined class="footer-back btn-xs-only" v-on:click="back(2)" v-show="icons > 1">
                <v-icon>mdi-store</v-icon>
                <span class="footer-font-size" style="text-transform: none"
                    >{{ $t("footer.shop") }}<span class="hidden-xs-only">{{ $t("footer.selection") }}</span></span
                >
            </v-btn>
            ...
            <v-btn outlined class="footer-back btn-xs-only" v-on:click="back(4)" v-show="icons > 3">
                ...
                <span class="footer-font-size" style="text-transform: none"
                    >&nbsp;{{ $t("footer.day") }}<span class="hidden-xs-only">{{ $t("footer.selection") }}</span></span
                >
            </v-btn>
        </v-col>
    </v-footer>
        ...
        back(icon) {
            switch (icon) {
                ...
                case 4:
                    ...
                    break
                case 9:
                    this.$router.push("/hairsalon/reservelist")
                    break
            }
        },
    },
