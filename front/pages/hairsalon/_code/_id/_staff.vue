        ...
        <v-container fluid style="margin-top: 82px">
            <v-row justify="center">
                <v-col class="ma-0 pa-0 text-center"
                    ><span style="font-weight: bold; color: red">{{ $t("utils.sun") }}</span></v-col
                >
                <v-col class="ma-0 pa-0 text-center"
                    ><span style="font-weight: bold">{{ $t("utils.mon") }}</span></v-col
                >
                <v-col class="ma-0 pa-0 text-center"
                    ><span style="font-weight: bold">{{ $t("utils.tue") }}</span></v-col
                >
                <v-col class="ma-0 pa-0 text-center"
                    ><span style="font-weight: bold">{{ $t("utils.wed") }}</span></v-col
                >
                <v-col class="ma-0 pa-0 text-center"
                    ><span style="font-weight: bold">{{ $t("utils.thu") }}</span></v-col
                >
                <v-col class="ma-0 pa-0 text-center"
                    ><span style="font-weight: bold">{{ $t("utils.fri") }}</span></v-col
                >
                <v-col class="ma-0 pa-0 text-center"
                    ><span style="font-weight: bold; color: blue">{{ $t("utils.sat") }}</span></v-col
                >
            </v-row>
        </v-container>
        <!-- Calendar -->
        <v-sheet width="100%" height="calc(100% - 151px)">
            ...
            >
                <template v-slot:day="context">
                        ...
                        <div v-if="isValid(context.date, minDate, maxDate)" v-bind:style="dayStyle(context.date, selectedMonth, minDate, maxDate)">
                            <v-hover v-slot:default="{ hover }" open-delay="150">
                                <v-card
                                    ...
                                    v-bind:elevation="hover ? 16 : 2"
                                    v-on:click="showDayDetail(context.date)"
                                >
                                    <v-card-text class="ma-0 pa-0 pt-lg-3 pt-1" style="font-size: 1em; color: black; white-space: nowrap">
                    ...
                    <div v-else style="width: 100%; height: 100%; background-color: #dcdcdc"></div>
                </template>
            </v-calendar>
        </v-sheet>
        ...
        <v-dialog v-model="dialog" fullscreen hide-overlay transition="dialog-bottom-transition">
            ...
            <v-dialog persistent max-width="460px" v-model="reserveDialog">
                <v-toolbar dense class="elevation-0">
                    ...
                    <div style="margin: auto; font-size: 1em">{{ $t("calendar.msg003") }}</div>
                </v-toolbar>
...
export default {
    ...
    async asyncData({ app, store, params, redirect }) {
        ...
        // 予約状況取得
        const schedulePromise = app.$hairsalon.getStaffMonthlySchedule(hairsalon.id, staff.staffId, months[0].value, {
            shop: hairsalon,
            staff: staff,
            menu: selectedMenu,
        })
    ...
    methods: {
        ...
        changeMonth(month) {
            ...
            // 担当者月別 予約状況取得
            this.$hairsalon.getStaffMonthlySchedule(hairsalonId, staffId, month, info).then((data) => {
                this.schedule = data
            })
        },
        ...
        async showDayDetail(date) {
            ...
            const staffId = this.staff.staffId
            schedule.events = await this.$hairsalon.getStaffDailySchedule(
                shopId,
                staffId,
                date,
                schedule,
