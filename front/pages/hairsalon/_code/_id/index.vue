    ...
    <v-app class="reserve-font-size">
        ...
        <v-dialog v-model="dialog" max-width="300">
            <v-card style="border-radius: 0">
                <v-card-title class="pl-4 pr-0 pb-0">
                    <div style="width: 100%">
                        <div v-if="selectedStaff.sex == 'male'">
                            <v-chip outlined left color="blue lighten-1" class="elevation-0">{{
                                $t("staffs.msg003", { career: selectedStaff.career })
                            }}</v-chip>
                            <span class="font-weight-bold ml-2" style="font-size: 1em">{{ selectedStaff.name }}</span>
                        </div>
                        <div v-else-if="selectedStaff.sex == 'female'">
                            <v-chip outlined color="pink lighten-1" class="elevation-0">{{
                                $t("staffs.msg003", { career: selectedStaff.career })
                            }}</v-chip>
                            <span class="font-weight-bold ml-2" style="font-size: 1em">{{ selectedStaff.name }}</span>
                        </div>
                        <div v-else class="text-center mr-6">
                            <span class="font-weight-bold" style="font-size: 1em">{{ selectedStaff.name }}</span>
                        </div>
                    </div>
                ...
                <v-card-title> <v-img style="width: 150px" v-bind:src="selectedStaff.img" /><br /> </v-card-title>
                <v-card-actions>
                ...
                <v-card-subtitle class="pl-3 pr-3 pb-1" v-show="selectedStaff.staffId != 0">
                    ...
                    <span style="color: #000">{{ $t("staffs.msg006") }}</span
                    ><br />
